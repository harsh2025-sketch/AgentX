...
:data:`~agentx.agent_loop.OrchestrationStatus.SUCCEEDED`, and it is
...
A2.10 must consume both the cognition-owned decision contracts (A2.06 Task
Manager, A2.07 Router, A2.08 Escalation, A2.09 anti-loop) and the
capabilities-owned execution/verification contracts (A1.10 runtime outcomes,
A2.05 Verifier). ``ALLOWED_ARCHITECTURE_EDGES`` in :mod:`agentx._architecture`
contains no ``(COGNITION, CAPABILITIES)`` edge — and no inbound edge to
``COGNITION`` at all — so **no canonical subsystem may legally import both
sides**. Widening the manifest, duplicating the canonical outcome/verifier
contracts, and substituting locally invented Protocols were all explicitly
rejected as architecture decisions.

This module therefore lives at the ``agentx`` namespace root, alongside the
existing top-level non-subsystem modules ``__main__``, ``_architecture``, and
``_version``. The canonical boundary checker states that "``agentx`` itself is
not a subsystem": ``_owning_subsystem`` returns ``None`` for the namespace
root and the checker only walks ``src/agentx/<subsystem>/``. A composition
root belongs *outside* the layers it composes — wiring is precisely the job
that cannot live inside either side without creating the subsystem
entanglement the manifest exists to prevent.

This is the smallest possible non-authority composition boundary: one module,
no new package, no new top-level subsystem, no manifest change. Critically,
``agentx.cognition`` still does not import ``agentx.capabilities``, and a
dedicated architecture test now pins that property.

Owner: A2.10. Adds no runtime dependency and widens no boundary edge.
"""
...
AGENT_LOOP_SOURCE: Final[str] = "agentx.agent_loop"
    ...
    * ``limits`` — the explicit A2.10 control-flow bounds plus the A2.09
    ...
    def __post_init__(self) -> None:
        ...
        if task.status is not TaskStatus.PENDING:
            raise OrchestrationRequestError(
                f"orchestration requires a Task in PENDING status; got {task.status.value}"
            )
    ...
    def __post_init__(self) -> None:
        ...
        if succeeded:
            if self.stop_reason is not OrchestrationStopReason.VERIFIED:
                raise OrchestrationRequestError("SUCCEEDED requires the VERIFIED stop reason")
            ...
            if self.stop_reason is OrchestrationStopReason.VERIFIED:
                raise OrchestrationRequestError("the VERIFIED stop reason requires SUCCEEDED")
...
def _verified_success(
    ...
    The parameters are deliberately typed ``object``: this gate sits on the
    boundary with caller-supplied strategy adapters, so the ``isinstance``
    checks below are load-bearing runtime guards rather than redundant
    assertions. Narrowing the annotations would make a static checker treat
    them as unreachable and invite their removal, which would let a forged
    outcome reach the success path.
    """
...
class AgentLoop:
    ...
    * a caller-supplied :class:`StrategyRegistry` of level adapters bound to
    ...
    def _classify(
        ...
        # Read the unwrapped value as ``object`` first: a caller-supplied
        # adapter can return anything at runtime, and this fail-closed guard
        # must survive static narrowing.
        unwrapped: object = outcome_result.unwrap()
        if not isinstance(unwrapped, ClosedLoopOutcome):
        ...
        if outcome_error is not None:
            classified = self._classify_error(outcome_error)
            if classified in (
                AttemptDisposition.RESOURCE_EXHAUSTED,
                AttemptDisposition.SAFETY_STOPPED,
            ):
                return (classified, outcome, evaluation, outcome_error)
        return (AttemptDisposition.UNVERIFIED, outcome, evaluation, outcome_error)
    ...
    def _finish(
        ...
        *,
        ...
        if transitioned.is_failure:  # pragma: no cover - RUNNING -> terminal is legal
            return Result[OrchestrationOutcome, AgentXError].failure(transitioned.unwrap_error())
    ...
    def _terminate_unavailable(
        ...
        *,
        ...
        error = _error(
            code="strategy_unavailable",
            message=(
                f"no authorized concrete strategy is registered for execution level {level.value}"
            ),
            category=ErrorCategory.PRECONDITION,
