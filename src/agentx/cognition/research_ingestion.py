"""Durable UNVERIFIED research ingestion into canonical Hive (AX-366).

Research findings are encoded as one deterministic inert payload inside the
canonical KnowledgeRecord so claim/evidence/retrieval/confidence metadata are
committed atomically by the existing KnowledgeStore row. No second truth store
or promotion path is introduced.
"""
...
class ResearchFinding:
    ...
    def __post_init__(self) -> None:
        ...
        environment = self.scope.value_for(ScopeDimension.ENVIRONMENT)
...
def decode_research_finding(record: KnowledgeRecord) -> ResearchFinding:
    """Decode a research payload without changing the record lifecycle state."""
    ...
    finding = ResearchFinding.from_dict(raw)
    if record.status is not KnowledgeStatus.UNVERIFIED:
        # Decoding remains possible for later lifecycle states, but ingestion
        # evidence must never itself be treated as re-verification.
        return finding
    return finding
