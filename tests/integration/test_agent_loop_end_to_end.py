...
:class:`~agentx.agent_loop.AgentLoop`.
...
def test_observation_exists_without_verification_and_is_not_success() -> None:
    ...
    outcome = loop.run(harness.make_request(limits=default_limits(max_total_attempts=1))).unwrap()
...
def test_denied_action_gate_path_never_executes_and_never_succeeds() -> None:
    ...
    outcome = loop.run(harness.make_request(limits=default_limits(max_total_attempts=2))).unwrap()
...
def test_emergency_stop_active_halts_orchestration_and_never_executes() -> None:
    ...
    outcome = loop.run(harness.make_request(limits=default_limits(max_total_attempts=4))).unwrap()
    ...
    assert any(item.outcome is AuditOutcome.STOP_REQUESTED for item in harness.audit_records)
...
def test_resource_exhaustion_fails_closed_and_never_widens_the_envelope() -> None:
    ...
    second = loop.run(harness.make_request(limits=default_limits(max_total_attempts=4))).unwrap()
    ...
    loop = harness.agent_loop(
        {
            ...
            if level is not ExecutionLevel.L5_EXPLORATORY
        }
    )
    request = harness.make_request(
        routing_evidence=RoutingEvidence(verified_reusable_result=True),
...
def test_cancellation_between_attempts_stops_the_governed_path() -> None:
    ...
    class CancelAfterFirst:
        ...
        def attempt(
            ...
            self.calls += 1
            result = self._inner.attempt(task, context, level)
...
def test_anti_loop_bounds_repeated_governed_failures() -> None:
    ...
    from agentx.agent_loop import OrchestrationLimits
