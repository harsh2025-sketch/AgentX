...
def failure(code: str, category: ErrorCategory):  # noqa: ANN201 - test factory
...
def test_verified_kind_without_verdict_is_not_success() -> None:
    ...
    def scripted(_attempt: int, _level: ExecutionLevel) -> StrategyResult:
        ...
        forged = ClosedLoopOutcome(
            ...
            error=None,
            execution=ExecutionResult(
                succeeded=True, message="forged", observation=observation
            ),
            observation=observation,
            verification=None,
            budget_usage=ResourceUsage.zero(),
        ...
        return StrategyResult.executed(
            Result[ClosedLoopOutcome, AgentXError].success(forged)
        )
...
def test_initial_l0_route_comes_from_a207() -> None:
    ...
    assert (
        ExecutionLevelRouter().route(request.routing_evidence).level is ExecutionLevel.L0_CACHE
    )
...
def test_no_de_escalation_ever_occurs() -> None:
    ...
    assert all(later >= earlier for earlier, later in zip(indices, indices[1:], strict=False))
...
def test_no_escalation_skipping_ever_occurs() -> None:
    ...
    indices = [CANONICAL_EXECUTION_LEVELS.index(r.level) for r in outcome.attempts]
    assert all(later - earlier == 1 for earlier, later in zip(indices, indices[1:], strict=False))
...
def test_task_terminal_transition_correctness(
    scenario: str, expected_status: TaskStatus
) -> None:
...
def test_two_loop_instances_agree_on_identical_inputs() -> None:
    ...
    for _ in range(2):
        ...
        loop = AgentLoop(
            ...
            strategies=StrategyRegistry({level: strategy for level in ExecutionLevel}),  # type: ignore[dict-item]
...
def test_module_reports_a_stable_source_identifier() -> None:
    ...
    assert AGENT_LOOP_SOURCE == "agentx.cognition.agent_loop"
