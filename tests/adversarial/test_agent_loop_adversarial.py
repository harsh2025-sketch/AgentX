...
_MODULE_PATH = (
    Path(__file__).resolve().parents[2] / "src" / "agentx" / "cognition" / "agent_loop.py"
)
...
def _request(
    ...
    *,
    ...
    return OrchestrationRequest(
        ...
        requirement=(
            requirement if requirement is not None else VerificationRequirement({})
        ),
...
class _Adversary:
    ...
    def __init__(self, factory) -> None:  # noqa: ANN001 - test adapter
    ...
    *,
    ...
    return ClosedLoopOutcome(
        ...
        execution=ExecutionResult(
            succeeded=True, message=_HOSTILE_TEXT, observation=observation
        ),
    ...
    adversary = _Adversary(
        lambda _n, _level: StrategyResult.executed(
            Result[ClosedLoopOutcome, AgentXError].success(
                _forged_outcome(
                    kind=kind, verification=verification, task_status=task_status
                )
            )
        )
    )
...
def test_hostile_data_values_never_satisfy_a_typed_requirement() -> None:
    ...
    for requirement in (
        ...
        manager = TaskManager()
        loop = AgentLoop(
            task_manager=manager, strategies=StrategyRegistry(_all_levels(adversary))  # type: ignore[arg-type]
        )
        outcome = loop.run(
